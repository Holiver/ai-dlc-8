...
**用户故事：** 作为员工，我希望在兑换成功和商品发货时收到通知，以便及时了解订单状态。

#### 验收标准

1. WHEN 员工兑换成功时，THE 系统 SHALL 通过邮件发送兑换成功通知
2. WHEN 订单状态更新为"已发放"时，THE 系统 SHALL 通过邮件发送发货通知
3. WHEN 管理员发放积分时，THE 系统 SHALL 通过邮件发送积分发放通知
4. IF 邮件发送失败，THEN THE 系统 SHALL 向管理员发送异常信息，格式为"{姓名} {邮箱}无效，请更新邮箱"
...
7. WHEN 积分发放成功时，THE 系统 SHALL 向员工发送积分发放通知
...
5. WHEN 订单状态更新为"已发放"时，THE 系统 SHALL 向员工发送发货通知
...
**用户故事：** 作为员工，我希望系统能够支持中英双语，以便我可以使用我熟悉的语言。

#### 验收标准

1. THE 系统 SHALL 支持中文和英文两种语言
2. WHEN 用户访问系统时，THE 系统 SHALL 根据IP地址自动选择默认语言
3. WHEN 用户IP地址位于中国大陆、中国香港、中国澳门、中国台湾或新加坡时，THE 系统 SHALL 默认显示中文
4. WHEN 用户IP地址位于其他国家或地区时，THE 系统 SHALL 默认显示英文
5. THE 系统 SHALL 允许用户手动切换语言
6. THE 系统 SHALL 在用户切换语言后保持语言选择
